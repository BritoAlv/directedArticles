...
Thus, PCA could be applied to transform the original vectors of an experiment with $n$ features, vectors with $d$ features. After this is done, those $d$ features are meaningless for the domain experts.

## Statistics POV

The variance of a set of vectors is the average of the squared distance of each vector to their mean.
...
If the vectors don't have too much variance, then they are close to the mean, and those $d$ dimensions don't separate them very well, which is not helpful for clustering them or distinguish them well.
