...
Now to find $R^{-1}$ is the inverse process, let's say we start with $v_1, v_2, \dots, v_d$ and end up with $o_1, o_2, \dots, o_d$
...
$v_2$ is defined by the second column of $R^{-1}$, in its first row, the pieces of $o_1$, we have to put $\langle v_2, o_1 \rangle$, in its second row we have to put $\|v_2 - \langle v_2, o_1 \rangle o_1\|$ the piece relative to $o_2$.
...
From this we get that it should be the case that $b^T A = x^T \implies x = A^T b$ which is true.

Now what if the $A$ matrix is not made by ortonormal vectors, how to proceed when that is the case.

We can write $A = QR$, where $Q$ is made by ortonormal vectors, and the subspace generated by $A$ is the same as
the one generated by $Q$, which means that:

If we want to find the vector $x$ it should be simply $Q^T b$, but this is from the $Q$ ortonormal vectors columns point of view.

To convert them back to the $A$ basis, we have to multiply it by $R^{-1}$, so that $x = R^{-1} Q^T b$
