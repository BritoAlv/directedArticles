...
The idea is that once that is known a set of orthonormal vectors, to add a new one, subtract from it its projection in the subspace formed by the orthonormal vectors.
...
This is a $d \times d$ matrix, the thing about it is that, it has columns $p_1, p_2, \dots, p_d$.
...
$R^{-1}$ exists because $R$ being upper triangular, means all of its vectors are linearly independent. It makes sense that it should exist, since one should be able to start with $Q$, apply all the linear transformations that were done and end up at $M$, in other words revert the process.
...
That expression is minimized, the solution is to find the vector $x$ such that $Ax$ is the projection of $b$ in that subspace.
...
The challenge there is to find the $x$, which is easy to do if the columns of $A$ are orthonormal vectors,
...
Now what if the $A$ matrix is not made by orthonormal vectors, how to proceed when that is the case.

Write $A = QR$, where $Q$ is made by orthonormal vectors, and the subspace generated by $A$ is the same as
