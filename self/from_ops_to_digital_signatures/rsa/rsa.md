...
One approach is that Bob has a way of producing locks that only he can unlock.

He gives locks to everyone that wishes to communicate with him, those people put the data on the lock, lock it, and send those to Bob somehow. Only Bob can unlock them.
...
A function $f$ can take an integer $m$ and map it to $f(m)$, if this function $f$ has properties that ensures that is no possible to get $m$ from $f$, that's where one-way comes from. They are useless in this setting because they allow encrypting the message, but not to decrypt it, unless somehow they have an exit-door that allows somehow get $m$ back.
...
With a number $n$, and a plain message $P$, there are the numbers:

$$P, P^2, \dots, P^{n-1} \mod n,$$

Public key (lock) will be one exponent $e$ and the modulus $n$, so to encrypt a message $P$, what's done is simply $P \rightarrow P^e \mod n$. To decrypt the message, is used other exponent $d$ that has the ideal property that $X^{de} = X \mod n$, so $P^{de} = P$.
...
This integer $d$ can be found using extended Euclid's algorithm if both $e$ and $\phi(n)$ are known, but the trick idea here is find a way of giving $n$ public, but ensuring that finding $\phi(n)$ not feasible from a computational point of view.
