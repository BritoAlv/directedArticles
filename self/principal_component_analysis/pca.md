...
See that $M'Mv$ will be vectors of dimension at most $d$, so the problem can be interpreted as finding the $d$ dimensional subspace that best approximates all the vectors in the dataset, i.e. $S$.

# How It Solves It:

Observe that $M'$ determines the subspace that is being looked for, because ultimately $M' (Mv)$ means write the vectors from the subspace generated by $M$, in terms of $M'$.

Fix $M'$, from the least square problem is known that the quantity $\|v - M' (Mv)\|^2$ is minimized when $Mv$ is the projection of $v$ into the subspace defined by $M'$.
...
Which happens when $M$ is precisely $(M')^T$.

Because $M'$ determines the subspace that's being looked for. This subspace can be represented using $d$ ortonormal vectors always.
...
Using trace operator, is possible go on, using the fact that if $B$, $C$ are such that $BC$ and $CB$ are square matrices, then $\text{tr}(BC) = \text{tr}(CB)$.
...
$tr(M S M')$ where $S$ is the sum matrix $(n \times n)$.
